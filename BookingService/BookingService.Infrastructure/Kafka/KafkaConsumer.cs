...
public class KafkaConsumer<T> : IKafkaConsumer<T>, IDisposable
{
    ...
    public KafkaConsumer(IOptions<KafkaSettings> settings, ILogger<KafkaConsumer<T>> logger)
    {
        ...
        // Уникальный GroupId на тип сообщения, чтобы независимые подписчики не мешали друг другу
        // (см. аналогичный комментарий в EventService.Infrastructure.Kafka.KafkaConsumer).
