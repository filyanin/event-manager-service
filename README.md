...
При запуске приложения контекст базы данных вызывает EnsureCreated(), поэтому схема базы данных (таблицы) создаётся автоматически при первом старте приложения. Для продакшен-окружения рекомендуется использовать миграции EF Core вместо EnsureCreated.
